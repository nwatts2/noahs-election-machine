...
function MyMap ({resultsYear, page, raceRecords, resultsRecords, setSenateCount, setGovCount, mode, setMode, isLoading}) {
                ...
                for (let x of raceRecords) {
                    if (x.type === mode && x.state.includes(thisState)) {
                        ...
                        for (let candidate of resultsRecords) {
                            if (candidate.year === resultsYear && candidate.state.includes(thisState)) {
                        ...
                        for (let race of raceRecords) {
                            if (race.year === resultsYear && race.state.includes(thisState) && race.type === mode) {
                                margin = race.margin;
                            }
                        }

                        currentRace = {state: x.state, called: called, hasElection: true, ratingRank: x.ratingRank, margin: margin};
                        break;
                    }
                }
    
                if (!currentRace.state) {
                    currentRace = {state: thisState, hasElection: false, called: ''};
                }
    
                newStates.push(currentRace);
            }
            if (mode === 'SENATE') {newStates.push({state: "OK-Special", called: '', hasElection: true, ratingRank: -3})}

        } else {
            for (let thisState of stateList) {
                let candidate = {}, altCandidate = {}, called = '', ratingRank, margin = 0;
    
                for (let x of resultsRecords) {
                    if (x.type === mode && x.state.includes(thisState) && x.year === resultsYear) {
                        candidate = x;
                        break;
                    }
                }
                ...
                for (let x of resultsRecords) {
                    if (x.type === mode && x.state.includes(thisState) && x.year === resultsYear && x !== candidate) {
                        altCandidate = x
                        break;
                    }
                }
                ...
                if (candidate !== {} && altCandidate !== {} && candidate.percent && altCandidate.percent) {
                    margin = Number(candidate.percent.slice(0, candidate.percent.length - 1)) - Number(altCandidate.percent.slice(0, altCandidate.percent.length - 1));
                ...
                let currentRace = {state: candidate.state, called: called, hasElection: true, ratingRank: ratingRank, margin: margin};
    
                if (!currentRace.state) {
                    currentRace = {state: thisState, hasElection: false, called: called};
                }
    ...
    function callRace(target) {
        const id = target.id;
        const currentState = states.find(item => item.state === id);
    ...
    function autoCallRaces(rating) {
        const solid = [], lean = [], likely = [], tilt = [], calledRaces=[];
    ...
    return (
        ...
        <br />
            <svg
                ...
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                ...
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MN' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MT' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                ...
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'ND' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'HI' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'ID' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'WA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'AZ' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'CA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'CO' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NV' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NM' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'OR' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'UT' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'WY' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'AR' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'IA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'KS' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MO' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NE' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'OK' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                {(mode === 'SENATE' && resultsYear === 2022) &&
                    <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'OK-Special' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                        className = 'mapState'
                        onClick={(e) => {handleClick(e)}}
                        onMouseEnter={(e) => {handleMouseEnter(e)}}
                        onMouseOut={(e) => {handleMouseLeave(e)}}
                        id="OK-Special"
                        data-name="Oklahoma Special"
                        data-id="OK-Special"
                        d='m 950,250 30,0 0,15 -30,0 0,-15 z'
                        style={{strokeWidth:strokeWidth}} /></Whisper>
                }   
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'SD' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'LA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'TX' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'CT' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NH' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'RI' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'VT' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'AL' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'FL' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'GA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MS' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'SC' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'IL' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'IN' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'KY' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NC' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'OH' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'TN' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'VA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'WI' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'WV' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'DE' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'DC' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MD' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NJ' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'NY' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'PA' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'ME' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'MI' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                    ...
                    style={{strokeWidth:strokeWidth}} /></Whisper>
                <Whisper trigger='hover'  speaker={<Popover className={(popupState !== 'AK' || enablePopups === false) ? 'invisible' : 'visible'}><Popup resultsYear={resultsYear} page={page} raceRecords={raceRecords} resultsRecords={resultsRecords} mode={mode} state={popupState} mouseposition={mouseposition}/></Popover>}>
                <path
                    className = 'mapState'
                    onClick={(e) => {handleClick(e)}}
                    onMouseEnter={(e) => {handleMouseEnter(e)}}
                ...
                {(mode === 'SENATE' && resultsYear === 2022) &&
                    <text id='OKtitle' textAnchor='middle' style={{fontSize: letterFont, fill: letterColor, stroke: letterColor}} x={1020} y={262}>OK-SPECIAL</text>
                }
